...
export default function App() {
  ...
  const initialValues = useRef({}) //initial values for the states (variables)
  ...
  const member = useRef(''); // Member name
  const adminPasscode = useRef(''); // Admin passcode
  const islevel = useRef('0'); // Member level
  ...
  const filteredParticipants = useRef([]); // Filtered participants
  ...
  const rtpCapabilities = useRef(null); // RTP capabilities from MediaSoup
  ...
  const redirectURL = useRef(`https://meeting.mediasfu.com/meeting/start`); // Redirect URL
  ...
  const onWaitingRoomFilterChange = (value) => {
    ...
    if (value !== '' && value.length > 0) {
      let filteredWaitingRoomList = waitingRoomList.current.filter((waitingRoom) => {
        return waitingRoom.name.toLowerCase().includes(value.toLowerCase());
      }
      );
      filteredWaitingRoomList.current = filteredWaitingRoomList;
      waitingRoomCounter.current = filteredWaitingRoomList.length;
    } else {
  ...
  const onRequestClose = () => {
    updateIsRsequestsModalVisible(false);
  }
  ...
  async function connect_Socket(apiUserName, apiKey, apiToken, link) {
    ...
    if (socket.current.id) {
      ...
      socket.current.on('disconnect', async () => {
        ...
        await disconnect({ parameters: { showAlert, redirectURL, onWeb: Platform.OS == 'web', updateValidated } });
